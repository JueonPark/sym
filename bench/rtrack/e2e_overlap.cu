...
Fixture makeFixture(const std::string &family, int64_t n, int64_t layers,
                    int64_t bAct) {
  Fixture f;
  f.n = n; f.layers = layers; f.bAct = bAct;
  if (family == "quant") {
  ...
  if (f.fam.s8) {
    ...
    for (size_t i = 0; i < xf.size(); ++i)
      xi[i] = (int8_t)std::lrintf(std::min(127.0f, std::max(-127.0f,
                                                            xf[i] * xInv)));
    CUDA_CHECK(cudaMalloc((void **)&f.dXi, xi.size()));
    CUDA_CHECK(cudaMemcpy(f.dXi, xi.data(), xi.size(),
                          cudaMemcpyHostToDevice));
...
void destroyFixture(Fixture &f) {
  ...
  if (f.dInv) cudaFree(f.dInv);
  if (f.dXi) cudaFree(f.dXi);
  if (f.dXf) cudaFree(f.dXf);
  if (f.dY) cudaFree(f.dY);
...
struct PipeCtx {
  ...
  void destroy() {
    for (auto &e : loadDone) cudaEventDestroy(e);
    for (auto &e : bufFree) cudaEventDestroy(e);
    for (auto &e : h2dTail) cudaEventDestroy(e);
...
void issueHostLoad(Fixture &f, PipeCtx &px, const PassCfg &cfg, int slot,
  ...
  const auto ck = bench::rtrack::planRowChunks(rows, outRowBytes,
                                               cfg.chunkBytes);
  const int64_t rowSrcBytes = f.rawBytes / rows;
  const int64_t minRows = std::max<int64_t>(
      1, (int64_t)reloc::kMinGatherBytesPerWorker /
    ...
    if (f.fam.s8) {
      pool.parallelFor(rb, re, minRows, [&](int64_t sb, int64_t se) {
        reloc::quant::quantizePackF32S8(
            f.hostW.data() + sb * f.n,
            (int8_t *)stage + (sb - rb) * f.n, se - sb, f.n,
            f.invScales.data() + sb, reloc::quant::Variant::Auto);
      });
    } else {
    ...
    if (cfg.compute) {
      ...
      for (int64_t rpt = 0; rpt < cfg.repeats; ++rpt) {
        if (f.fam.s8)
          ...
          g.sgemm((const float *)f.dW[slot], f.dXf, (float *)f.dY, f.n,
                  f.bAct);
      }
  ...
  for (int it = 0; it < kWarmup + kIters; ++it) {
    ...
    for (int64_t k = 0; k < f.layers; ++k)
      for (int64_t rpt = 0; rpt < repeats; ++rpt) {
        if (f.fam.s8)
          g.igemm((const int8_t *)f.dW[0], f.dXi, (int32_t *)f.dY, f.n,
                  f.bAct);
        else
...
void writeRow(std::ofstream &csv, const Args &a, const Fixture &f,
  ...
  const double delta =
      (loadOnly > 0 || computeOnly > 0)
          ? wall.median - std::max(loadOnly, computeOnly) : 0.0;
  const double exposed = computeOnly > 0 ? wall.median - computeOnly : 0.0;
  csv << a.machine << ",2080ti," << method << ',' << f.fam.name << ',' << f.n
      << ',' << num(f.fam.r) << ",8," << num((double)chunkBytes / (1 << 20))
      << ',' << nChunks << ',' << f.layers << ',' << f.bAct << ',' << repeats
      << ',' << num(measuredC) << ',' << num(wall.median) << ','
      << num(wall.min) << ',' << num(wall.p95) << ','
...
int main(int argc, char **argv) {
  ...
  const std::vector<double> chunksMib =
      a.chunksMib.empty()
          ? (f.fam.s8 ? std::vector<double>{4, 16}
                      : std::vector<double>{16, 64})
          : splitDoubles(a.chunksMib);
  const std::vector<double> bChunksMib =
      a.chunksMib.empty() ? std::vector<double>{256, 16}
                          : splitDoubles(a.chunksMib);
  int64_t maxStage = 0;
  for (double c : chunksMib) {
    const int64_t rows = f.fam.bound.extents[0];
    const auto ck = bench::rtrack::planRowChunks(
        rows, f.wireBytes / rows, (int64_t)(c * (1 << 20)));
    maxStage = std::max(maxStage, ck.stagingBytes);
  }
  f.stagingBytes = maxStage;
  for (int s = 0; s < 2; ++s)
    CUDA_CHECK(cudaHostAlloc(&f.staging[s], (size_t)maxStage,
                             cudaHostAllocDefault));
  ...
  PassCfg gemCfg; gemCfg.loader = Loader::None; gemCfg.repeats = 1;
  const double tGemmLayer = quick(gemCfg) / (double)a.layers;
  PassCfg la; la.loader = Loader::A; la.compute = false;
  ...
  struct Anchor { RStats only, bare; };
  std::vector<Anchor> anchors(repeatsGrid.size());
  for (size_t i = 0; i < repeatsGrid.size(); ++i) {
    PassCfg co; co.loader = Loader::None; co.repeats = repeatsGrid[i];
  ...
  // 3. Loader matrix.
  struct M { const char *name; Loader l; bool serial; };
  ...
  for (const M &m : all) {
    if (a.methods != "all" &&
        a.methods.find(m.name) == std::string::npos)
      continue;
    const std::vector<double> &grid =
        (m.l == Loader::BPipelined) ? bChunksMib
        : (m.l == Loader::APrefold) ? std::vector<double>{64}
                                    : chunksMib;
    for (double cMib : grid) {
      PassCfg cfg;
      cfg.loader = m.l; cfg.serial = m.serial;
      cfg.chunkBytes = (int64_t)(cMib * (1 << 20));
      // load-only anchor for this (method, chunk).
      PassCfg lo = cfg; lo.compute = false; lo.serial = false;
      ...
      const int64_t nCh =
          (m.l == Loader::BPipelined)
              ? bench::rtrack::planByteChunks(f.rawBytes, cfg.chunkBytes)
                    .nChunks
              : (m.l == Loader::APrefold)
                    ? bench::rtrack::planByteChunks(f.wireBytes,
                                                    cfg.chunkBytes).nChunks
                    : bench::rtrack::planRowChunks(
                          f.fam.bound.extents[0],
                          f.wireBytes / f.fam.bound.extents[0],
                          cfg.chunkBytes).nChunks;
      ...
      for (size_t i = 0; i < repeatsGrid.size(); ++i) {
        ...
        const double mC =
            loadOnly.median > 0 ? anchors[i].only.median / loadOnly.median
                                : 0;
        bool v = !a.verify || verifyConfig(f, (int)((f.layers - 1) & 1),
                                           refY);
