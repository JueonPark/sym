#!/usr/bin/env python3
"""CM6 (issue #125): register the corrected model's predictions (m_eff
composition + fill/drain active + cold recv basis + Gen4 hbm
re-baseline). **Post-hoc refinement, not a pre-registration** — the BP
dataset pre-exists (#124); the defense is zero tuning freedom: every
input is a code constant, a pre-declared derivation, or a CM1
measurement. Supersedes bench/results/cm4_registered_predictions.json
(frozen). v3_gate.py's CM6-REGEN re-runs this script and byte-compares.

  PYTHONPATH=build/sym/python python3 bench/rtrack/cm6_register.py \
      --out bench/results/cm6_registered_predictions.json
...
# CM6 (issue #125): rstar predictions are keyed on pattern, so families
# sharing a pattern get IDENTICAL curves (the CM5-review finding). Record
# the degeneracy on every rstar row instead of letting a future measured
# family silently inherit its twin's prediction.
PATTERN_DEGENERATE = {
    fam: sorted(o for o, (_, p2) in FAMILY_MAP.items()
                if o != fam and p2 == pattern)
    for fam, (_, pattern) in FAMILY_MAP.items()
}
        ...
        for fam in sorted(FAMILY_MAP):
            ...
            for placement in PLACEMENTS:
                ...
                rstar.append({"box": box, "family": fam,
                              ...
                              "pattern_degenerate_with": PATTERN_DEGENERATE[fam]})

    doc = {
        "generated_by": "bench/rtrack/cm6_register.py",
        "issue": ("#125 (CM6); post-hoc refinement over the BP dataset "
                  "(#116/#124); supersedes #112's registration (frozen)"),
        "model": "reloc::costmodel as of CM1(#109)+CM2(#110)+CM3(#111)+CM6(#125)",
