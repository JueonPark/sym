...
| `.to(float16)` | `to` | `aten._to_copy.default` | Same-device cast | No; `typed_transform_unavailable` (the numerical contract is defined by C1 in [reloc-typed-semantics.md](reloc-typed-semantics.md); the artifact is C3's wire v1 typed plan and R3 executes it through `reloc_torch.dispatch`; the FX import gate is C4's) |
