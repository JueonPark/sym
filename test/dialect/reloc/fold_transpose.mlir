...
// reports it as a remark on the chain tail. Plans carry no constraints and
// no_copy = false: constraint emission belongs to #B2/#B3, no_copy
// detection to #B5.
...
func.func @single_transpose(%t: !sym.tensor<[6, 4, 2], f32>) -> !sym.tensor<[2, 6, 4], f32> {
  // expected-remark @below {{folded plan: #reloc.plan<src = tensor<[6, 4, 2], f32>, dst = tensor<[2, 6, 4], f32>, perm = [2, 0, 1], axes = [{name = "d2", extent = 2, src_stride = 1, dst_stride = 24}, {name = "d0", extent = 6, src_stride = 8, dst_stride = 4}, {name = "d1", extent = 4, src_stride = 2, dst_stride = 1}], inverse = affine_map<(d0, d1, d2) -> (d1, d2, d0)>>}}
...
func.func @composed_transposes(%t: !sym.tensor<[6, 4, 2], f32>) -> !sym.tensor<[2, 4, 6], f32> {
  %0 = reloc.transpose %t perm [2, 0, 1] : !sym.tensor<[6, 4, 2], f32> -> !sym.tensor<[2, 6, 4], f32>
  // expected-remark @below {{folded plan: #reloc.plan<src = tensor<[6, 4, 2], f32>, dst = tensor<[2, 4, 6], f32>, perm = [2, 1, 0], axes = [{name = "d2", extent = 2, src_stride = 1, dst_stride = 24}, {name = "d1", extent = 4, src_stride = 2, dst_stride = 6}, {name = "d0", extent = 6, src_stride = 8, dst_stride = 1}], inverse = affine_map<(d0, d1, d2) -> (d2, d1, d0)>>}}
...
func.func @inverse_pair(%t: !sym.tensor<[6, 4, 2], f32>) -> !sym.tensor<[6, 4, 2], f32> {
  %0 = reloc.transpose %t perm [2, 0, 1] : !sym.tensor<[6, 4, 2], f32> -> !sym.tensor<[2, 6, 4], f32>
  // expected-remark @below {{folded plan: #reloc.plan<src = tensor<[6, 4, 2], f32>, dst = tensor<[6, 4, 2], f32>, perm = [0, 1, 2], axes = [{name = "d0", extent = 6, src_stride = 8, dst_stride = 8}, {name = "d1", extent = 4, src_stride = 2, dst_stride = 2}, {name = "d2", extent = 2, src_stride = 1, dst_stride = 1}], inverse = affine_map<(d0, d1, d2) -> (d0, d1, d2)>>}}
...
func.func @symbolic_extents(%t: !sym.tensor<[6, "N", 2], f32>) -> !sym.tensor<["N", 6, 2], f32> {
  // expected-remark @below {{folded plan: #reloc.plan<src = tensor<[6, N, 2], f32>, dst = tensor<[N, 6, 2], f32>, perm = [1, 0, 2], axes = [{name = "d1", extent = N, src_stride = 2, dst_stride = 12}, {name = "d0", extent = 6, src_stride = 2 * N, dst_stride = 2}, {name = "d2", extent = 2, src_stride = 1, dst_stride = 1}], inverse = affine_map<(d0, d1, d2) -> (d1, d0, d2)>>}}
