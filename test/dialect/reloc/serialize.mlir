...
// Issue #56: a NON-ZERO fill (bits 0x11223344) on the outer axis. Every
// other golden fills with 0.0, so this is the one wire blob that proves
// the full 64-bit fill field survives encode -> decode -> bind -> execute
// (libreloc/test/ExecuteTest.cpp, NonZeroFillGoldenRoundTripsEndToEnd).
// CHECK: plan_hex(pad_nonzero): 
// expected-remark @below {{encoded 307 bytes, deterministic = true, within_size_budget(4096) = true}}
"test.plan"() {serialize, name = "pad_nonzero", plan = #reloc.plan<src = tensor<[2, 3], f32>, dst = tensor<[4, 3], f32>, perm = [0, 1], axes = [{name = "r", extent = 2, src_stride = 3, dst_stride = 3}, {name = "c", extent = 3, src_stride = 1, dst_stride = 1}], pad_fill = [{dst_axis = 0, lo = 1, hi = 1, value = 0x11223344 : f32}], inverse = affine_map<(d0, d1) -> (d0, d1)>>} : () -> ()
