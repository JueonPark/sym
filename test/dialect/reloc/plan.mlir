...
func.func @reference_plan() {
  // CHECK: "test.use_attr"() {plan = #reloc.plan<src = tensor<[N, N], f32, strides = [N, 1]>, dst = tensor<[N floordiv 64, 64, 64, N floordiv 64], f32>, perm = [1, 0, 2, 3], axes = [{name = "b0", extent = 64, src_stride = N, dst_stride = N * (N floordiv 64)}, {name = "n0", extent = N floordiv 64, src_stride = 64 * N, dst_stride = N}, {name = "b1", extent = 64, src_stride = N floordiv 64, dst_stride = N floordiv 64}, {name = "n1", extent = N floordiv 64, src_stride = 1, dst_stride = 1}], constraints = {divisible(N, 64), contiguous = [false, false, false, true], no_copy = false}, inverse = #map>}
  "test.use_attr"() {plan = #reloc.plan<src = tensor<[N, N], f32, strides = [N, 1], offset = 0>, dst = tensor<[N floordiv 64, 64, 64, N floordiv 64], f32>, perm = [1, 0, 2, 3], axes = [{name = "b0", extent = 64, src_stride = N, dst_stride = N * (N floordiv 64)}, {name = "n0", extent = N floordiv 64, src_stride = 64 * N, dst_stride = N}, {name = "b1", extent = 64, src_stride = N floordiv 64, dst_stride = N floordiv 64}, {name = "n1", extent = N floordiv 64, src_stride = 1, dst_stride = 1}], pad_fill = [], constraints = {divisible(N, 64), contiguous = [false, false, false, true], no_copy = false}, inverse = affine_map<(d0, d1, d2, d3) -> (d1, d0, d2, d3)>>} : () -> ()
  return
}
